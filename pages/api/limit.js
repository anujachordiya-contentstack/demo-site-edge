...
export default async function handler(req, res) {
  ...
  // Extract endpoints from request body
  let apiEndpoints = [];

  if (Array.isArray(req.body)) {
    apiEndpoints = req.body;
  } else if (req.body && Array.isArray(req.body.endpoints)) {
    apiEndpoints = req.body.endpoints;
  } else {
    return res.status(400).json({
      message: 'Invalid request body. Expected an array of endpoints or { endpoints: [...] }',
  ...
  if (apiEndpoints.length === 0) {
    return res.status(400).json({
      message: 'No endpoints provided. Please provide at least one endpoint URL.'
    });
  }

  const invalidEndpoints = apiEndpoints.filter(url => typeof url !== 'string');
  if (invalidEndpoints.length > 0) {
    return res.status(400).json({
      message: 'All endpoints must be strings (URLs)',
      invalidCount: invalidEndpoints.length
    });
  }

  console.log("Starting POST calls to all API endpoints");
  console.log(`Received ${apiEndpoints.length} endpoints to process`);
